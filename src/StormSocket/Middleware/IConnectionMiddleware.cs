...
public interface IConnectionMiddleware
{
    ...
    ValueTask OnConnectedAsync(ISession networkSession) => ValueTask.CompletedTask;
    ...
    /// </summary>
    ValueTask<ReadOnlyMemory<byte>> OnDataReceivedAsync(ISession networkSession, ReadOnlyMemory<byte> data) => ValueTask.FromResult(data);

    /// <summary>
    /// Called before data is sent to the client.
    /// Return modified data to send, or <see cref="ReadOnlyMemory{T}.Empty"/> to suppress.
    /// </summary>
    ValueTask<ReadOnlyMemory<byte>> OnDataSendingAsync(ISession networkSession, ReadOnlyMemory<byte> data) => ValueTask.FromResult(data);

    /// <summary>Called after a session disconnects. Middlewares are called in reverse order.</summary>
    ValueTask OnDisconnectedAsync(ISession networkSession, DisconnectReason reason) => ValueTask.CompletedTask;

    /// <summary>Called when an exception occurs during connection handling.</summary>
    ValueTask OnErrorAsync(ISession networkSession, Exception exception) => ValueTask.CompletedTask;
}