...
public delegate ValueTask SessionConnectedHandler(ISession networkSession);

/// <summary>Fired when a TCP session disconnects.</summary>
public delegate ValueTask SessionDisconnectedHandler(ISession networkSession, DisconnectReason reason);

/// <summary>Fired when raw data (or a framed message) is received from a TCP session.</summary>
public delegate ValueTask DataReceivedHandler(ISession networkSession, ReadOnlyMemory<byte> data);
...
public delegate ValueTask WsMessageReceivedHandler(ISession networkSession, WsMessage message);

/// <summary>Fired when a WebSocket client completes the upgrade handshake.</summary>
public delegate ValueTask WsConnectedHandler(ISession networkSession);

/// <summary>Fired when a WebSocket client disconnects.</summary>
public delegate ValueTask WsDisconnectedHandler(ISession networkSession, DisconnectReason reason);
